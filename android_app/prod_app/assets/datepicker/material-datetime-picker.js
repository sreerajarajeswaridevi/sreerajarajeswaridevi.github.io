setTimeout(() => {
  ...
  triggerElement.classList.add('rr-datepicker-trigger');
  triggerElement.style.display = 'none';


  var createDatePickerEvent = new CustomEvent(
    "createDatePicker",
    {
      detail: {
        datePickerRef: MaterialDatetimePicker
      },
      bubbles: true,
      cancelable: true
    }
  ...
  triggerElement.addEventListener('click', function() {
    triggerElement.dispatchEvent(createDatePickerEvent);
  }, false);
...
var DateTimePicker = function (_Events) {
  ...
  function DateTimePicker() {
    ...
    if (_this.options.startDate || _this.options.endDate) {
      _this.startDate = _this.options.startDate || moment(0);
      _this.endDate = _this.options.endDate || moment(8640000000000000);

      _this.dateValidator = function (date) {
        return !_this.startDate.clone().startOf('day').isAfter(date) && !_this.endDate.clone().endOf('day').isBefore(date);
      };

      _this.timeValidator = function (date) {
        return !_this.startDate.isAfter(date) && !_this.endDate.isBefore(date);
      };
    }
    return _this;
  }
  ...
  createClass(DateTimePicker, [{
    ...
    key: '_dateIsBetween',
    value: function _dateIsBetween(startDate, endDate, date) {
    ...
    key: 'setDateInsideLimits',
    value: function setDateInsideLimits() {
    ...
    key: 'disableDateOptions',
    value: function disableDateOptions(type) {
      if (this.timeValidator) {
        switch (type) {
          case 'ampm':
            {
              if (this.startDate || this.endDate) {
                var amStart = this.value.clone().startOf('day');
                var amEnd = amStart.clone().add(12, 'hours').add(-1, 'minutes');

                var pmStart = amStart.clone().add(12, 'hours');
                var pmEnd = pmStart.clone().add(12, 'hours').add(-1, 'minutes');

                if (this._dateIsBetween(this.startDate, this.endDate, amStart) || this._dateIsBetween(this.startDate, this.endDate, amEnd)) {
                ...
                if (this._dateIsBetween(this.startDate, this.endDate, pmStart) || this._dateIsBetween(this.startDate, this.endDate, pmEnd)) {
          ...
          case 'hours':
            {
              ...
              var hours = this.$('.c-datepicker__clock__hours .c-datepicker__clock__num');
              for (var i = 0; i < hours.length; i += 1) {
                var date = this.value.clone().startOf('hour').hour(parseInt(hours[i].getAttribute('data-number'), 10) + hoursOffset);
          ...
          case 'minutes':
            {
              var minutes = this.$('.c-datepicker__clock__minutes .c-datepicker__clock__num');
              for (var _i = 0; _i < minutes.length; _i += 1) {
                var _date = this.value.clone().minute(minutes[_i].getAttribute('data-number'));
                if (this.timeValidator(_date)) {
                  minutes[_i].classList.remove('c-datepicker__clock__num--disabled');
                } else {
                  minutes[_i].classList.add('c-datepicker__clock__num--disabled');
                }
              }
              break;
            }
